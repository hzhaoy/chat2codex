...
import { normalizeChatType, type ChatType, type SenderIdentity } from "../../core/access-control.js";
import type { IncomingAttachment, IncomingTextMessage } from "../../core/message-router.js";
...
export function extractSenderIdentity(event: unknown): SenderIdentity {
  ...
  const openId = getString(senderId, "open_id") ?? getString(messageSenderId, "open_id");
  const userId = getString(senderId, "user_id") ?? getString(messageSenderId, "user_id");
  const unionId = getString(senderId, "union_id") ?? getString(messageSenderId, "union_id");
  return {
    keys: [
      openId ? { kind: "open_id", value: openId } : undefined,
      userId ? { kind: "user_id", value: userId } : undefined,
      unionId ? { kind: "union_id", value: unionId } : undefined,
    ].filter((key): key is { kind: string; value: string } => Boolean(key)),
    // Retain v0.6 aliases while old persisted inbox records remain readable.
    openId,
    userId,
    unionId,
  };
